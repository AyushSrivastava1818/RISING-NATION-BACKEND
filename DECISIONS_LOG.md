...
## Delivery Scope: Backend-Only

- **SCOPE-DECISION**: this build is a backend-only deliverable. `frontend/` contains only a stub (`package.json` + a one-paragraph `README.md`) and was never implemented — every slice across the commit history (scaffold through deployment/CI) built exclusively the four backend layers, schema/migrations, and now deployment config. This wasn't a scope reduction made partway through; the root `README.md` has titled this repo "Rising Nation — Backend" and described it as "Backend API for Rising Nation" since the initial commit.
  - **Why recorded here**: `ENGINEERING.md`/`ARCHITECTURE.md`/`DECISIONS_LOG.md`'s own Tech Stack table above recommend Next.js for the frontend — that recommendation was never withdrawn, it's simply out of scope for what was actually built in this repo. Recording the scope boundary explicitly (rather than leaving it implied by an empty `frontend/` folder) matters for handoff: without this entry, a future reader could reasonably wonder whether frontend work was attempted and abandoned, or deliberately never started. It was the latter.
  - **What a frontend team needs**: `docs/` (`REQUIREMENTS.md`, `ARCHITECTURE.md`, `DATABASE.md`, `API.md`, `ENGINEERING.md`) is the original specification this backend was built against, not backend-specific documentation — it defines the full product (including frontend-facing requirements, page/flow descriptions, and the API contract in `API.md`) and is the correct starting point for a frontend build, alongside this backend's actual implemented behavior (`README.md`'s "What's implemented" table, `PROGRESS.md`) where the two diverge (e.g., `opportunities` is Post-MVP and has no backend routes yet to build a frontend against).
...
- **SCHEMA-DEVIATION**: `categories.slug` added beyond DATABASE.md spec. Reason: URL-safe unique identifier for clean web routing. Not present in original spec because: `DATABASE.md` ERD used `name` as the sole text attribute.
  - **Correction (handoff audit)**: this entry is wrong and is kept only for the record trail. `DATABASE.md`'s Entity Specification for `categories` (not just its ERD diagram, which never lists individual fields for any table) already lists `slug | string | No | — | UNIQUE — stable URL/filter key`. `categories.slug` is not a deviation — it matches spec exactly. No corresponding code or spec change needed.
- **SCHEMA-DEVIATION**: `users.password_hash` added beyond DATABASE.md spec. Reason: Secure bcrypt authentication for admin accounts per `ARCHITECTURE.md` §3.6. Not present in original spec because: `DATABASE.md` ERD omitted authentication credential fields.
  - **Correction (handoff audit)**: the "added beyond spec" framing is wrong — `DATABASE.md`'s Entity Specification for `users` already lists `password_hash | string | No | — | bcrypt; never logged`, i.e. `NOT NULL`. The actual, still-unresolved deviation is nullability: `schema.prisma` implements it as `password_hash String?` (nullable), and `src/services/auth.service.ts` defensively checks `!user.password_hash` at every login/reset-related read (lines checking it before `verifyPassword`/`createPasswordResetToken`/`verifyPasswordResetToken`), meaning the nullable case is actively handled, not accidental. No code path in `src/` currently creates a `User` without a `password_hash`, though `user.repository.ts`'s `CreateUserData.password_hash` is typed optional. Left as an open, unresolved deviation rather than "corrected" either direction — tightening to `NOT NULL` forecloses a plausible future no-password member-account path (`ENGINEERING.md` OD-1), and there's no evidence either choice was deliberately decided.
- **SCHEMA-DEVIATION**: `users.updated_at` added beyond DATABASE.md spec. Reason: Track account password and profile update timestamps. Not present in original spec because: `DATABASE.md` ERD omitted lifecycle timestamp columns.
  - **Correction (handoff audit)**: this entry is wrong and is kept only for the record trail. `DATABASE.md`'s Entity Specification for `users` already lists `created_at` / `updated_at | timestamptz | No | now()`. `users.updated_at` is not a deviation — it matches spec exactly. No corresponding code or spec change needed.
